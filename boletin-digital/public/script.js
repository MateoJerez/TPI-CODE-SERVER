(() => {
  ...
  let API_BASE = (function(){
    try {
      ...
      if (port === '3000') return `${proto}//${host}${port ? (':' + port) : ''}`;
  ...
  async function detectApiBase(candidates = [
    ...
    for (const base of candidates) {
      if (!base) continue;
      try {
        const pingUrl = new URL('/api/ping', base).toString();
        const res = await fetch(pingUrl, { method: 'GET', mode: 'cors', credentials: 'include' });
        if (res && (res.ok || res.status === 200 || res.status === 204)) {
          API_BASE = base.replace(/\/$/, '');
          console.info('API base detectada (ping):', API_BASE);
          return API_BASE;
        }
      } catch (e) {
        try {
          const meUrl = new URL('/api/auth/me', base).toString();
          const r2 = await fetch(meUrl, { method: 'GET', credentials: 'include' });
          if (r2) {
            API_BASE = base.replace(/\/$/, '');
            console.info('API base detectada (auth):', API_BASE);
            return API_BASE;
          }
        } catch (e2) {
  ...
  function ensureInputIsPassword(el) {
    if (!el) return null;
    try {
      if ('type' in el) {
        el.type = 'password';
  ...
  function findLoginLink() {
    const byId = document.getElementById('link-login');
    if (byId) return byId;

    const byHref = document.querySelector('nav a[href*="login.html"], .nav-inner a[href*="login.html"]');
    if (byHref) return byHref;

    const byText = Array.from(document.querySelectorAll('nav a')).find(a => /Iniciar Sesi[oó]n/i.test(a.textContent) || /\bLogin\b/i.test(a.textContent) || /\bSalir\b/i.test(a.textContent));
    return byText || null;
  }
  ...
  function ensureLogoutButtonInNav() {
    ...
    if (navInner) navInner.appendChild(bt);
    else if (nav) nav.appendChild(bt);
  ...
  async function loadInviteIfNeeded() {
    try {
      const res = await fetchWithCred(apiUrl('/api/auth/invite'), { method: 'GET' });
      if (!res.ok) return null;
      const data = await safeJson(res);
      if (data && data.ok && data.invite) return data.invite;
      return null;
    } catch (e) {
      // ignore
      return null;
  ...
  function showInviteSection(visible, sectionId = 'inviteSection') {
    const sec = document.getElementById(sectionId);
    if (!sec) return;
    sec.style.display = visible ? 'block' : 'none';


    const inviteInput = sec.querySelector('input[name="invite_code"], input[id^="invite_code"]');
    if (inviteInput) {
      ensureInputIsPassword(inviteInput);
      if (!visible) inviteInput.value = '';
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (registerForm) {
      ...
      if (idRolSelect) {
        idRolSelect.addEventListener('change', () => {
          const val = Number(idRolSelect.value || 0);

          showInviteSection(val === 2 || val === 3, 'inviteSection');
        });

        showInviteSection(Number(idRolSelect.value || 0) === 2 || Number(idRolSelect.value || 0) === 3, 'inviteSection');
      }

      const invite = await loadInviteIfNeeded().catch(()=>null);
      if (invite) {
        let inviteInput = document.getElementById('invite_code');
        if (inviteInput) {
          inviteInput = ensureInputIsPassword(inviteInput);
          inviteInput.value = invite;
        }
        const inviteInfo = document.getElementById('inviteInfo');
        if (inviteInfo) { inviteInfo.textContent = `Código de invitación prellenado (oculto)`; inviteInfo.style.display = 'block'; }
      }
    }
    ...
    if (loginForm) {
      ...
      if (loginRoleSelect) {
        loginRoleSelect.addEventListener('change', () => {
          const val = Number(loginRoleSelect.value || 0);

          showInviteSection(val === 2 || val === 3, 'inviteSectionLogin');
        });
        showInviteSection(Number(loginRoleSelect.value || 0) === 2 || Number(loginRoleSelect.value || 0) === 3, 'inviteSectionLogin');
      }
      const inviteLogin = await loadInviteIfNeeded().catch(()=>null);
      if (inviteLogin) {
        let inviteInputLogin = document.getElementById('invite_code_login');
        if (inviteInputLogin) {
          inviteInputLogin = ensureInputIsPassword(inviteInputLogin);
          inviteInputLogin.value = inviteLogin;
        }
      }
    }
    ...
    async function renderBoletin(alumnoId, opts = {}) {
      ...
      if (!alumnoId && sessionUsuario && Number(sessionUsuario.id_rol) === 1) {
        alumnoId = sessionUsuario.id ?? sessionUsuario.id_alumno ?? sessionUsuario.alumno_id ?? alumnoId;
      }
    ...
    async function verificarSesion() {
      try {
        ...
        let res = null;
        try {
          res = await fetchWithCred(apiUrl('/api/auth/me'), { method: 'GET' });
        } catch (err) {

          if (linkLogin) linkLogin.style.display = 'inline-block';
          ensureLogoutButtonInNav().style.display = 'none';
          _cachedUser = null;
          return null;
        }
        ...
        if (data && (data.ok || data.success) && data.usuario) {
          ...
          const logoutBtn = ensureLogoutButtonInNav();
    ...
    (function attachLoginHandler() {
      ...
      loginForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (usuario) {
            _cachedUser = usuario; // cache
            const logoutBtn = ensureLogoutButtonInNav();
            logoutBtn.style.display = 'inline-block';
            if (linkLogin) linkLogin.style.display = 'none';
            attachLogoutHandlerRobust('#logoutBtn');

            const roleToUse = usuario?.id_rol ?? id_rol;
          ...
          const roleToUse2 = maybeData?.usuario?.id_rol ?? id_rol;
          if (Number(roleToUse2) === 1) window.location.href = '/boletin.html';
          else if (Number(roleToUse2) === 2) window.location.href = '/alumnado.html';
          else window.location.href = '/admin.html';
        } catch (err) {
  ...
  function attachAlumnoToBoletinClicks(containerSelector) {
    ...
    const containerSelectOnly = cont.getAttribute('data-select-only') === 'true';
    const isAlumnadoPage = window.location.pathname.includes('/alumnado');

    cont.addEventListener('click', (ev) => {

      const elWithDataId = ev.target.closest('[data-id]');
      let id = null;
      if (elWithDataId) id = elWithDataId.getAttribute('data-id') || elWithDataId.dataset.id;
      else {
      ...
      if (isAlumnadoPage || containerSelectOnly) {
        ev.preventDefault();

        const tbody = cont.querySelector('tbody') || cont;
        if (tbody) {
          Array.from(tbody.querySelectorAll('tr')).forEach(r => r.classList.remove('selected'));
        }
        const clickedRow = elWithDataId?.closest('tr') || ev.target.closest('tr');
        ...
        try {
          const event = new CustomEvent('alumno:selected', { detail: { id } });
